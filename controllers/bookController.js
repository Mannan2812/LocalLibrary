...
exports.book_delete_get = function(req, res,next) {
    async.parallel({
      ...
      if(err) return next(err);
      res.render('book_delete',{title:'Book Delete',book:results.book,instance:results.instance})
    })
};
...
exports.book_delete_post = function(req, res) {
  async.parallel({
    ...
    if(err) return next(err);
    else if(results.instance.length)
    res.render('book_delete',{title:'Book Delete',book:results.book,instance:results.instance})
    else{
      Book.findByIdAndRemove(req.body.bookid).exec(function (err){
        if(err) return next(err)
